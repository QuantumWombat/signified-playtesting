...
class Testbed():
    ...
    def __init__(self):
        ...
        starting_companion_types = random.choices(list(CompanionType), k=NUM_STARTING_COMPANIONS)
    ...
    def reroll_cards(self):
        ...
        self.shop.generate_card_options([c.companion_type for _, c in self.companions.items()])
...
class ShopLevel(Enum):
    ...
    ONE = (1, ShopConstants(num_cards_to_show=3, num_companions_to_show=2, upgrade_cost=4))
    TWO = (2, ShopConstants(num_cards_to_show=4, num_companions_to_show=3, upgrade_cost=8))
...
class Shop():
    ...
    def upgrade(self) -> None:
        if self.gold < self.level.shop_constants.upgrade_cost:
            raise ValueError(f"cannot afford card cost of {self.level.shop_constants.upgrade_cost}, you have {self.gold}")
    ...
    def generate_card_options(self, current_companions: List[CompanionType]) -> None:
        ...
        self.card_options = random.choices(card_opts, k=min(len(card_opts), self.level.shop_constants.num_cards_to_show))

    def generate_companion_options(self) -> None:
        self.companion_options = random.choices(list(CompanionType), k=self.level.shop_constants.num_companions_to_show)
    ...
    def _buy_card(self, card: Card) -> None:
        ...
        if self.gold < CARD_COST:
            raise ValueError(f"cannot afford card cost of {CARD_COST}, you have {self.gold}")
    ...
    def _buy_companion(self, companion: CompanionType) -> None:
        ...
        if self.gold < COMPANION_COST:
            raise ValueError(f"cannot afford companion cost of {COMPANION_COST}, you have {self.gold}")
    ...
    def __str__(self) -> str:
        disp = [
            ...
            f"COMPANIONS ({COMPANION_COST} gold): " + ", ".join([c.value for c in self.companion_options]),
            f"CARDS ({CARD_COST} gold): " + ", ".join([card.value for card in self.card_options]),
